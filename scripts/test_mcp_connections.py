...
def test_github(token: str | None) -> dict[str, any]:
    ...
    try:
        ...
        return {"status": "error", "message": f"Ошибка подключения: {e!s}"}


def test_postgres(connection_string: str | None) -> dict[str, any]:
    ...
    if connection_string.startswith("sqlite"):
        ...
        try:
            ...
            with engine.connect() as conn:
                ...
                result.fetchone()
            return {"status": "success", "message": "SQLite подключение успешно", "type": "sqlite"}
        except Exception as e:
            return {"status": "error", "message": f"Ошибка SQLite: {e!s}"}
    else:
        ...
        try:
            ...
            return {"status": "error", "message": f"Ошибка PostgreSQL: {error_msg}"}
...
def test_redis(url: str | None) -> dict[str, any]:
    ...
    try:
        ...
        return {"status": "error", "message": f"Ошибка Redis: {e!s}"}
...
def main():
    ...
    postgres_url = os.getenv("POSTGRES_CONNECTION_STRING") or env_vars.get(
    ...
    status_icon = (
        "✅"
        if results["postgres"]["status"] == "success"
        else "❌"
    ...
    status_icon = (
        "✅"
        if results["docker"]["status"] == "success"
        else "❌"
        if results["docker"]["status"] == "error"
        else "⏭️"
    )
    ...
    if error_count > 0:
        ...
        print(
            "   2. Запустите скрипт из WSL: wsl -d Ubuntu -e bash -c 'cd /home/alex/fastapi-project && python3 scripts/test_mcp_connections.py'"
        )
